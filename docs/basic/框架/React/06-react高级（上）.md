...
因此可能会写出这样的代码，看起来很合理，在调用完 `setState` 之后，输出 `count`，发现显示的 `count` 和我们控制台输出的 `count` 值是不一样的。
...
我们将 `setState` 填上第二个参数，输出更新后的 `count` 值，这样我们就能成功的获取到最新的数据了，如果有这个需求我们可以在第二个参数输出噢~

### 1.2 函数式 setState

函数式 `setState` 接收两个参数：

- `updater` ，它是一个能够返回 `stateChange` 对象的函数

- `callback()` 回调函数，用于在状态更新完毕，界面也更新之后调用

与对象式不同的是，我们传递的第一个参数 `updater` 可以接收到 2 个参数 `state` 和 `props`
...
我们编写组件的时候每次都需要采用一个 `div` 标签包裹，才能让它正常的编译，但是这样会引发什么问题呢？---- 组件树中包裹了好几层无意义的 div 标签。

我们可以采用 `Fragment` 来解决这个问题。
...
在 React 中为我们提供了一种更加便捷的方式，直接使用`<></>`代替 Fragment 更加简单，它又叫做幽灵标签：
